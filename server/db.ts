import { eq, desc, and, like, count, gte, sql } from "drizzle-orm";
import { drizzle } from "drizzle-orm/mysql2";
import { InsertUser, users, translations, signDictionary, sessionHistory, userPreferences, fingerspellingAlphabet, InsertSignDictionary } from "../drizzle/schema";
...
export async function saveTranslation(userId: number, inputText: string, glossSequence: string, language: 'ASL' | 'ISL', sourceType: 'text' | 'audio' | 'youtube') {
  ...
  return db.insert(translations).values({ userId, inputText, glossSequence, language, sourceType });
}
...
export async function getSignByGloss(gloss: string, language: 'ASL' | 'ISL') {
  ...
  const result = await db.select().from(signDictionary)
    .where(and(eq(signDictionary.gloss, gloss.toUpperCase()), eq(signDictionary.language, language)))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function searchDictionary(query: string, language: 'ASL' | 'ISL', category?: string, limit: number = 20) {
  const db = await getDb();
  if (!db) return [];
  const conditions = [eq(signDictionary.language, language)];
  if (query) conditions.push(like(signDictionary.gloss, `%${query.toUpperCase()}%`));
  if (category) conditions.push(eq(signDictionary.category, category));
  return db.select().from(signDictionary).where(and(...conditions)).limit(limit);
}
...
export async function getOrCreateUserPreferences(userId: number) {
  ...
  await db.insert(userPreferences).values({ userId, preferredLanguage: 'ASL', avatarChoice: 'default', playbackSpeed: 100 });
  return { id: 0, userId, preferredLanguage: 'ASL' as const, avatarChoice: 'default', playbackSpeed: 100, createdAt: new Date(), updatedAt: new Date() };
}
...
export async function getFingerspellingLetter(letter: string, language: 'ASL' | 'ISL') {
  ...
  const result = await db.select().from(fingerspellingAlphabet)
    .where(and(eq(fingerspellingAlphabet.letter, letter.toUpperCase()), eq(fingerspellingAlphabet.language, language)))
    .limit(1);
  return result.length > 0 ? result[0] : null;
}
...
export async function getTranslationById(translationId: number) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(translations).where(eq(translations.id, translationId)).limit(1);
  return result.length > 0 ? result[0] : null;
}
